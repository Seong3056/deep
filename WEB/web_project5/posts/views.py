...
def post_add(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            # Post의 "user" 값은 request에서 자져와 자동할당한다
            ...
            tag_string = request.POST.get("tags")  # 해시태그가 없는경우도 있기 떄문에 .get사용
            if tag_string:
                tag_name_list = [tag_name.strip() for tag_name in tag_string.split(",")] # strip:좌우 공백 없애기, split(""): "안에있는 걸 기준으로 쪼개기"
                for tag_name in tag_name_list:
                    tag, _ = HashTag.objects.get_or_create(    # 변수명에 _쓰면 사용하지 않을 변수라는 의미
                        name=tag_name,
                    )
...
def tags(request, tag_name):
    try:
        ...
        # tag_name에 해당하는 HashTag를 찾지 못한 경우 빈 QuertSet을 반환
    ...
    # context로  Template에 필터링 된 Post QuertSet 을 넘겨주며
    # 어떤 tag_name으로 검색했는지도 넘겨 줌
    context = {
        "tag_name" : tag_name,
        "posts" : posts,
    }
...
def post_like(request, post_id):
    ...
    # 전달되지 않았다면 피드페이지에서 해당 Post위치로 ㅣㅇ동
